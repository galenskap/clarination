...
const SESSION_AUTHORIZED_KEY = 'clarina.mic_authorized'
...
const devices: Ref<MicDevice[]> = ref([])
const selected_device_id: Ref<string | null> = ref(read_stored_device_id())

let subscriber_count = 0
let session_authorized = read_session_authorized()
let ensure_promise: Promise<MediaStream | null> | null = null
...
function read_session_authorized(): boolean {
  try {
    return sessionStorage.getItem(SESSION_AUTHORIZED_KEY) === '1'
  } catch {
    return false
  }
}

function mark_session_authorized() {
  session_authorized = true
  try {
    sessionStorage.setItem(SESSION_AUTHORIZED_KEY, '1')
  } catch {
...
async function is_browser_permission_granted(): Promise<boolean> {
  try {
...
/**
 * Réouvre le micro sans nouveau geste si l’accès a déjà été accordé
 * (cette session ou permission navigateur déjà « granted »).
 */
async function ensure_access(): Promise<MediaStream | null> {
  if (status.value === 'granted' && stream.value) {
    return stream.value
  }
  ...
  ensure_promise = (async () => {
    if (session_authorized || (await is_browser_permission_granted())) {
...
function stop() {
  ...
  stream.value = null
  if (status.value === 'granted' || status.value === 'requesting') {
    status.value = 'idle'
  }
  stop_tracks(current)
}
